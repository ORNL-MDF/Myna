...
def main(argv=None):
    ...
    datatype = myna.metadata.return_datatype_class(
        settings["data"]["build"]["datatype"]
    )
    database_path = build_path
    if datatype == myna.metadata.PeregrineDB:
    ...
    for i, step in enumerate(settings["steps"]):
        ...
        for data_req in step_obj.data_requirements:
            ...
            data_class_name = myna.metadata.return_data_class_name(data_req)
            constructor = vars(myna.metadata)[data_class_name]

            # Construct the relevant data object
            if constructor.__base__ == myna.metadata.BuildMetadata:
            ...
            elif constructor.__base__ == myna.metadata.PartMetadata:
            ...
            elif constructor.__base__ == myna.metadata.BuildFile:
                data_obj = constructor(datatype, build_path)
                data_obj.copy_file()
                datum = {
            ...
            elif constructor.__base__ == myna.metadata.PartFile:
                for part in parts.keys():
                    data_obj = constructor(datatype, build_path, part)
                    data_obj.copy_file()
                    datum = {
                    ...
                    settings["data"]["build"]["parts"][part][data_req] = datum
            elif constructor.__base__ == myna.metadata.LayerFile:
                for part in parts.keys():
